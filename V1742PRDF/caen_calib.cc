...
void CAEN_Calib::apply_sgl_time_calib(Packet *p, const int feech)
{
  ...
  //
  // Sort by time (it's possible for the delay line to make a correction so that
  // a later sample actually occurs earlier in time than a previous sample
  //
  vector< pair<float,float> > vect;
  for (int isamp=0; isamp<1024; isamp++)
  {
    vect.push_back( make_pair(time_corr[feech][isamp],adc_corr[feech][isamp]) );
  }
  sort(vect.begin(), vect.end());
  for (int isamp=0; isamp<1024; isamp++)
  {
    /*
    // Check if there are any samples with out of time ordering
    if ( isamp==0 ) cout << "*************" << endl;
    if ( time_corr[feech][isamp] != vect[isamp].first )
    {
      cout << isamp << "\t" << time_corr[feech][isamp] << "\t" << vect[isamp].first << endl;
    }
    */

    adc_corr[feech][isamp] = vect[isamp].second;
    time_corr[feech][isamp] = vect[isamp].first;
  }
